...
export class ProductService implements IProductService {
  ...
  async confirmUpload(
    ...
    const backendBaseUrl =
      process.env.APP_BASE_URL || `http://localhost:${appConfig?.port ?? 3000}`;
  ...
  async findAll(
    ...
    this.logger.log(
      `Finding all products${page && limit ? ` (page: ${page}, limit: ${limit})` : ''}`,
    );
    const options = page && limit ? { page, limit } : undefined;
    return await this.productRepository.findAll(options);
  }

  async findByIds(ids: number[], userId?: number | null): Promise<Product[]> {
    this.logger.log(`Finding products by IDs: ${ids.join(', ')}`);
    if (!ids || ids.length === 0) {
      return [];
    }
  ...
  async search(searchQuery: string, userId?: number | null): Promise<Product[]> {
    ...
    if (exactProductIds.length > 0) {
      ...
      products = Array.from(new Map(partialProducts.map((p) => [p.id, p])).values());
      this.logger.log(`Found ${products.length} products with partial match`);
    }

    this.logger.log(`Found ${products.length} products matching search criteria`);
    return products;
  }
